...
if [ -f "$GOPATH/src/github.com/IBM/ibmcloud-volume-file-vpc/Passing" ]; then
	curl https://img.shields.io/badge/e2e-passing-Yellow.svg > $TRAVIS_BUILD_DIR/gh-pages/coverage/$TRAVIS_BRANCH/e2e.svg
elif [ -f "$GOPATH/src/github.com/IBM/ibmcloud-volume-file-vpc/Failed" ]; then
...
if [ "$TRAVIS_PULL_REQUEST" == "false" ]; then
	...
	git push origin
        ...
        # Updates PR with coverage
   		curl -X POST -H "Authorization: Token $GHE_TOKEN" "https://api.github.com/repos/$GHE_USER/$TRAVIS_REPO_SLUG/issues/$TRAVIS_PULL_REQUEST/comments" -H 'Content-Type: application/json' --data '{"body": "'"$RESULT_MESSAGE"'"}'
