...
var _ = Describe("[ics-e2e] [rfs-snapshot] VPC FILE RFS PROFILE | Dynamic Provisioning and Snapshot", func() {
	f := framework.NewDefaultFramework("ics-e2e-vpcfile-rfs-snap")
	f.NamespacePodSecurityEnforceLevel = admissionapi.LevelPrivileged
	...
	It("should run snapshot lifecycle tests for RFS VPC File", func() {
		...
		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: RFS PROFILE | SNAPSHOT | RESTORE SAME SIZE | DELETE SNAPSHOT: PASS\n"); err != nil {
			panic(err)
		}
		...
		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: RFS PROFILE | SNAPSHOT | RESTORE CLAIM SIZE LESS | DELETE SNAPSHOT AFTER SOURCE VOLUME DELETION: PASS\n"); err != nil {
			panic(err)
		}
		...
		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: RFS PROFILE | SNAPSHOT | RESTORE CLAIM SIZE MORE | DELETE SNAPSHOT: PASS\n"); err != nil {
			panic(err)
		}

		// TEST 4 — SNAPSHOT UNATTACHED MUST FAIL
		By("VPC-FILE-CSI-TEST: RFS PROFILE | SNAPSHOT FOR UNATTACHED VOLUME MUST FAIL")
		test1.SnapShotForUnattached(cs, snapshotrcs, ns)

		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: RFS PROFILE | SNAPSHOT FOR UNATTACHED VOLUME MUST FAIL: PASS\n"); err != nil {
			panic(err)
		}
	})
})

var _ = Describe("[ics-e2e] [dp2-snapshot] VPC FILE DP2 PROFILE | Dynamic Provisioning and Snapshot", func() {
	f := framework.NewDefaultFramework("ics-e2e-vpcfile-dp2-snap")
	f.NamespacePodSecurityEnforceLevel = admissionapi.LevelPrivileged
	...
	It("should run snapshot lifecycle tests for DP2 VPC File", func() {
		...
		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: DP2 PROFILE | SNAPSHOT | RESTORE SAME SIZE | DELETE SNAPSHOT: PASS\n"); err != nil {
			panic(err)
		}
		...
		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: DP2 PROFILE | SNAPSHOT | RESTORE CLAIM SIZE LESS | DELETE SNAPSHOT AFTER SOURCE VOLUME DELETION: PASS\n"); err != nil {
			panic(err)
		}
		...
		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: DP2 PROFILE | SNAPSHOT | RESTORE CLAIM SIZE MORE | DELETE SNAPSHOT: PASS\n"); err != nil {
			panic(err)
		}

		// TEST 4 — SNAPSHOT UNATTACHED MUST FAIL
		By("VPC-FILE-CSI-TEST: DP2 PROFILE | SNAPSHOT FOR UNATTACHED VOLUME MUST FAIL")
		test1.SnapShotForUnattached(cs, snapshotrcs, ns)

		if _, err = fpointer.WriteString("VPC-FILE-CSI-TEST: DP2 PROFILE | SNAPSHOT FOR UNATTACHED VOLUME MUST FAIL: PASS\n"); err != nil {
			panic(err)
		}
	})
})
