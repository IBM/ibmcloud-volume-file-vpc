...
if [[ "$IC_LOGIN" != "true" ]]; then
    CONFIGPATH="./e2e/scripts/statefulset-test/kube-config/vpc"
    CWDIR=$(pwd)
    ...
    if [[ ! -e ./e2e/scripts/statefulset-test/${TESTSCRIPT}  ]]; then
    	echo "********** E2E Statefulset Test Details **********" > $E2E_TEST_SETUP
    	echo -e "StartTime   : $(date "+%F-%T")" >> $E2E_TEST_SETUP
    	echo -e "Error       : Setup failed" >> $E2E_TEST_SETUP
		echo -e "Error       : Testscript not found ./e2e/scripts/statefulset-test/${TESTSCRIPT}" >> $E2E_TEST_SETUP
    	echo -e "Error       : Unbale to execute e2e test!" >> $E2E_TEST_SETUP
    	echo -e "VPC-FILE-CSI-TEST: Statefulset test: FAILED" > $E2E_TEST_RESULT
    	exit 1
    fi
...
echo "                  Path: `pwd`"
chmod 755 ./e2e/scripts/statefulset-test/${TESTSCRIPT}
echo "Info: ./e2e/scripts/statefulset-test/${TESTSCRIPT}"
./e2e/scripts/statefulset-test/${TESTSCRIPT} | tee -a $E2E_TEST_RESULT
