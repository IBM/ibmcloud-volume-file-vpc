...
func TestListSecurityGroups(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			content:   "{\"errors\":[{\"message\":\"testerr\",\"Code\":\"list_not_found\"}], \"trace\":\"2af63776-4df7-4970-b52d-4e25676ec0e4\"}",
			expectErr: "Trace Code:2af63776-4df7-4970-b52d-4e25676ec0e4, Code:list_not_found, Description:testerr, RC:404 Not Found",
		}, {
			...
			name: "Verify that resource_group.id and VPCID is added to the query",
			filters: &models.ListSecurityGroupFilters{
			...
			status: http.StatusNoContent,
			muxVerify: func(t *testing.T, r *http.Request) {
				expectedValues := url.Values{"resource_group.id": []string{"rgid"}, "vpc.id": []string{"vpc-1"}, "version": []string{models.APIVersion}}
				actualValues := r.URL.Query()
				assert.Equal(t, expectedValues, actualValues)
			},
		},
	...
	for _, testcase := range testCases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			test.SetupMuxResponse(t, mux, vpcfilevolume.Version+"/security_groups", http.MethodGet, nil, testcase.status, testcase.content, nil)
			...
			securityGroups, err := shareFileService.ListSecurityGroups(testcase.limit, testcase.start, testcase.filters, logger)
			logger.Info("securityGroups", zap.Reflect("securityGroups", securityGroups))
