...
func TestCreateFileShareTarget(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			name:    "Verify that the share target creation is done correctly",
			...
			verify: func(t *testing.T, shareTarget *models.ShareTarget, err error) {
				if assert.NotNil(t, shareTarget) {
					assert.Equal(t, "share target id", shareTarget.ID)
					assert.Equal(t, "xvdc", shareTarget.VPC.ID)
					assert.Equal(t, "161.26.114.179:/nxg_s_volle91789b5_54d0_4cd3_87ab_776683355ec0/dd843c3b-7523-4940-aa5b-9c89119f6515", shareTarget.MountPath)
				}
			},
		},
	}
	...
	for _, testcase := range testCases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			shareTarget, err := shareService.CreateFileShareTarget(template, logger)
			logger.Info("Volume target details", zap.Reflect("shareTarget", shareTarget))

			if testcase.expectErr != "" && assert.Error(t, err) {
				assert.Equal(t, testcase.expectErr, err.Error())
				assert.Nil(t, shareTarget)
			} else {
