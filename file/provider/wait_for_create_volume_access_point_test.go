...
func TestWaitForCreateVolumeAccessPoint(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			testCaseName: "Volume Access Point exist for the VPCID but timeout occurs",
			providerVolumeAccessPointRequest: provider.VolumeAccessPointRequest{
				VolumeID: "volume-id1",
				VPCID:    "VPC-id1",
			},
	...
	for _, testcase := range testCases {
		t.Run(testcase.testCaseName, func(t *testing.T) {
			...
			volumeAccessPoint, err := vpcs.WaitForCreateVolumeAccessPoint(testcase.providerVolumeAccessPointRequest)
